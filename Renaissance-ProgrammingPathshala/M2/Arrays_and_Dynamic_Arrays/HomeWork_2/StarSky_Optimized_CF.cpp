...
void printAllSkyMoments(vector<NumMatrix> &skyAtMoments, int topLeftX, int topLeftY, int bottomRightX, int bottomRightY)
{
  ...
  {
    cout<<"moment: "<<momentNumber<<"\n";
    for(int i=topLeftX;i<=bottomRightX;i++)
    {
      for(int j=topLeftY;j>=bottomRightY;j--)
      {
        cout<<"("<<i<<","<<j<<")"<<skyAtMoment.tempMatrix[i][j]<<" ";
      }
      cout<<"\n";
    }
...
int main()
{
  int n, q, c;
  cin>>n>>q>>c;
  vector<vector<int>> sky = vector(MAX_STAR_COUNT + 1, vector<int>(MAX_STAR_COUNT + 1,-1));
  int pos[n][2];
  ...
  for(int t=0;t<=c;t++)
  {
    vector<vector<int>> temp = vector(MAX_STAR_COUNT + 1, vector<int>(MAX_STAR_COUNT + 1, 0));
    for(int i=0;i<n;i++)
    {
      temp[pos[i][0]][pos[i][1]] = (sky[pos[i][0]][pos[i][1]]+t)%(c+1);
    }
    skyAtMoments.push_back(NumMatrix(temp));
  }
...
/*
Input 0:
Renaissance-ProgrammingPathshala/M2/Arrays_and_Dynamic_Arrays/HomeWork_2/StarSky_Optimized_CF.cpp
9 7 5
1 1 0
1 2 0
1 3 0
